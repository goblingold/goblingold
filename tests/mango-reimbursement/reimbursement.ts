...
describe("mango reimbursement", () => {
  ...
  it("Mango reimbursement USDC", async () => {
  ...
  it("Mango reimbursement RAY", async () => {
    ...
    let tx = await program.mangoReimbursement(6, new BN(15826)); // 16587
    await program.provider.sendAndConfirm(tx, [], CONFIRM_OPTS);
  });

  it("Mango reimbursement mSOL", async () => {
    program.setToken("mSOL");
    let tx = await program.mangoReimbursement(10, new BN(18614)); //19629
    await program.provider.sendAndConfirm(tx, [], CONFIRM_OPTS);
  });

  it("Mango reimbursement USDT", async () => {
    program.setToken("USDT");
    let tx = await program.mangoReimbursement(4, new BN(20158)); //21320
    await program.provider.sendAndConfirm(tx, [], CONFIRM_OPTS);
  });
});
